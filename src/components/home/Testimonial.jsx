...
const Testimonial = () => {
  ...
  React.useEffect(() => {
    (async () => {
      const res = await http("/reviews");
      const data = await res.json();
      setTestimoni(data.result);
      console.log(data.result);
    })();
  }, []);

  const prev = () => {
    if (testimoni.length === 0) return;
  ...
  const next = () => {
    ...
    setIndex((prev) => (prev === testimoni.length - 1 ? 0 : prev + 1));
  };
          ...
          <div className="flex w-full flex-col items-center lg:flex-row">
            ...
            <article className="w-full px-6 py-12 text-center sm:px-10 md:px-16 lg:w-1/2 lg:px-32 lg:py-24 lg:text-left">
              <div className="mx-auto max-w-xl lg:mx-0">
                ...
                <h2 className="border-l-4 border-orange-400 pl-4 text-3xl leading-tight font-bold text-white sm:text-4xl md:text-5xl lg:border-l-8 lg:pl-6">
                  {item.Name}
                </h2>
                <p className="mt-4 text-sm font-semibold tracking-wide text-orange-400 uppercase sm:text-lg">
                  {item.AuthorTitle}
                </p>
                <p className="mt-8 px-4 text-sm leading-relaxed font-light text-white italic opacity-80 sm:text-lg lg:mt-10 lg:px-0 lg:leading-loose line-clamp-1">
                  {item.Message}
                </p>
                ...
                <div className="mt-7 flex justify-center gap-4 lg:justify-start lg:gap-6">
                  <button
                    className="group flex h-12 w-12 transform items-center justify-center rounded-full bg-white shadow-lg transition-all duration-300 hover:scale-110 hover:bg-orange-400 active:scale-95 lg:h-14 lg:w-14"
                    onClick={prev}
                  >
                    <img
                      src={ArrowLeft}
                  ...
                  <button
                    className="group flex h-12 w-12 transform items-center justify-center rounded-full bg-orange-400 shadow-lg transition-all duration-300 hover:scale-110 hover:bg-white active:scale-95 lg:h-14 lg:w-14"
                    onClick={next}
                  >
                    <img
                      src={ArrowRight}
